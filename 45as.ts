...
while (invitees.length > 2) {
  let removedInvitee = invitees.pop();
  console.log(`Sorry, ${removedInvitee}, we can't have you at dinner.`);
}
...
// T-Shirt Function
function makeShirtt(size: string, message: string): void {
  ...
  // Big Shirts
  function createLargeShirtt(message: string = "I Love TypeScript!"): void {
    console.log(`The shirt is size Large with the message: "${message}".`);
  }
  ...
  function describeCityy(city: string, country: string = "USA"): void {
  ...
  // City and Country
  function cityAndCountryy(city: string, country: string): string {
    return `${city}, ${country}`;
  }
  ...
  // Music Album
  function createMusicAlbumm(artist: string, albumName: string, tracks?: number): object {
    let album = { artist, albumName };
  ...
  // Magicians
  let magiciansList: string[] = ["David Copperfield", "Criss Angel", "Derren Brown"];
  magiciansList.forEach(magician => console.log(magician));
  
  function makeThemGreatt(magicians: string[]): string[] {
    let greatMagicians: string[] = [];
    magicians.forEach(magician => greatMagicians.push(`The Amazing ${magician}`));
    return greatMagicians;
  }
  
  let greatMagicians = makeThemGreatt([...magiciansList]);
  ...
  // Sandwich Order
  function makeSandwichh(...ingredients: string[]): void {
  ...
  // Cars Function
  function createCarr(manufacturer: string, modelName: string, ...features: { [key: string]: any }[]): object {
    let car = { manufacturer, modelName, features: {} };
  ...
  function buildProfilee(firstName: string, lastName: string, ...extraInfo: { [key: string]: any }[]): object {
    let profile = { firstName, lastName, ...extraInfo[0] }; // Ensure extraInfo is properly accessed
    return profile;
  }
